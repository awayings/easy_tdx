...
class TestDescribeUnusualKnownTypes:
    ...
    def test_type_0x04(self):
        ...
        desc, val = _describe_unusual(0x04, bytes.fromhex("00b8d73d3d0000000000000000"), 9)
        assert desc == "加速拉升"
        assert val == "4.63%"

    def test_unknown_type_fallback(self):
        desc, val = _describe_unusual(0x42, bytes.fromhex("00" * 13), 9)
...
class TestType0x15:
    ...
    def test_auction_drop(self):
        ...
        desc, val = _describe_unusual(0x15, bytes.fromhex("030c9846bc003e1d4700000000"), 9)
        assert desc == "竞价下跌"
        assert val == "-1.21%/40254手"

    def test_auction_rise(self):
        # 真实样本：600127 金健米业 09:25:00，v1=0x02 竞价拉升（尾段自 10.84 冲至 12.05）
        desc, val = _describe_unusual(0x15, bytes.fromhex("0213d2cd3d00367b4700000000"), 9)
        assert desc == "竞价拉升"
        assert val == "10.05%/64310手"

    def test_auction_flat(self):
        # 真实样本：600410 华胜天成 09:25:01，v1=0x01 竞价平稳（尾段价格未动）
        desc, val = _describe_unusual(0x15, bytes.fromhex("01000000000098a54500000000"), 9)
        assert desc == "竞价平稳"
    ...
    def test_unknown_sub_type_falls_back(self):
        desc, _ = _describe_unusual(0x15, struct.pack("<B2fI", 0x77, 0.0, 100.0, 0), 9)
        assert desc == "竞价异动"
...
class TestType0x16:
    ...
    def test_strong_at_auction(self):
        ...
        desc, val = _describe_unusual(0x16, bytes.fromhex("010f506e3dcb846e3d00000000"), 9)
        assert desc == "盘中强势"
        assert val == "5.82%"

    def test_weak_at_auction(self):
        # 真实样本：600683 京投发展 09:25:01，v1=0xFF（弱势 1 级），v2=-6.40%
        desc, val = _describe_unusual(0x16, bytes.fromhex("ffc71d83bd690383bd00000000"), 9)
        assert desc == "盘中弱势"
        assert val == "-6.40%"

    def test_new_stock_no_limit(self):
        # 真实样本：601123 N马矿 09:25:00，新股无涨跌幅限制，v2=+245.86%
        desc, val = _describe_unusual(0x16, bytes.fromhex("03775a1d404a5b1d4000000000"), 9)
        assert desc == "盘中强势"
        assert val == "245.86%"
...
class TestType0x13:
    ...
    def test_auction_test_buy(self):
        ...
        desc, val = _describe_unusual(0x13, bytes.fromhex("00295c0b41006c354600000000"), 9)
        assert desc == "竞价试买"
        assert val == "8.71/11611手"

    def test_auction_test_sell(self):
        # 真实样本：603900 09:15:17，申报价 6.46 低于昨收 7.17（往下试）
        desc, val = _describe_unusual(0x13, bytes.fromhex("0152b8ce400000c94300000000"), 9)
        assert desc == "竞价试卖"
...
class TestType0x1D0x1E:
    ...
    def test_fast_rise(self):
        ...
        desc, val = _describe_unusual(0x1D, bytes.fromhex("009d50843c0000000000000000"), 9)
        assert desc == "急速拉升"
        assert val == "1.62%"

    def test_fast_fall(self):
        # 真实样本：601123 N马矿 09:35:03
        desc, val = _describe_unusual(0x1E, bytes.fromhex("019cd393bc0000000000000000"), 9)
        assert desc == "急速下跌"
        assert val == "-1.80%"
...
class TestTypeNames:
    ...
    def test_mapped_types_produce_named_desc(self):
        ...
        for utype in UNUSUAL_TYPE_NAMES:
            desc, _ = _describe_unusual(utype, zeros, 9)
